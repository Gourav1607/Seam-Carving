...
def sc_resize_image(img, nshape):
    ...
    for i in range(vdiff):
        print('Removing VSeams - {:03d}/{:03d} - ({:03d},{:03d})'.format(
            i+1, vdiff, img_out.shape[0], img_out.shape[1]), end='\r')
    ...
    for j in range(hdiff):
        print('Removing HSeams - {:03d}/{:03d} - ({:03d},{:03d})'.format(
            j+1, hdiff, img_out.shape[0], img_out.shape[1]), end='\r')
...
def display_seam(img, seam, vflag=True):
    ...
    for i, pt in enumerate(seam):
        print(i, end='\r')
        if vflag:
            newimg = cv2.circle(newimg, (i, pt), radius=1,
                                color=(0, 0, 255), thickness=1)
        else:
            newimg = cv2.circle(newimg, (pt, i), radius=1,
                                color=(0, 0, 255), thickness=1)
    ...
    if vflag:
        plt.savefig('display_seam_v.png', dpi=300,
                    bbox_inches='tight', pad_inches=0.2)
    else:
        plt.savefig('display_seam_h.png', dpi=300,
                    bbox_inches='tight', pad_inches=0.2)
    plt.show()
...
print('The energy image has a resolution of (X Y) : ',
      energy.shape[0], energy.shape[1])
nshape = input(
    'Enter New Resolution for Image (X and Y seperated by space) : ')
nshape = nshape.split()
...
plt.tight_layout()
plt.savefig('SeamCarving_Full.png', dpi=300,
            bbox_inches='tight', pad_inches=0.2)
plt.show()
